...
void handle_float_parameter(CInstantCamera& camera, string name, double value)
{
        INodeMap& nodemap = camera.GetNodeMap();
        try
        {
...
void handle_enum_parameter(CInstantCamera& camera, string name, string value)
{
        INodeMap& nodemap = camera.GetNodeMap();
        try
        {
...
void handle_basler_parameter(CInstantCamera& camera, XmlRpc::XmlRpcValue& param)
{
        ...
        if ("int" == type)
        {
                ...
                handle_int_parameter(camera, param["name"], param["value"]);
        }
        else if ("float" == type)
        {
                ...
                handle_float_parameter(camera, param["name"], param["value"]);
        }
        else if ("enum" == type)
        {
                ...
                handle_enum_parameter(camera, param["name"], param["value"]);
        }
        else
        {
...
void load_parameters(CInstantCamera& camera)
{
...
void configure_callback(basler_camera::CameraConfig &config, uint32_t level)
{
        ...
        handle_float_parameter(camera, "AcquisitionFrameRate", config.frame_rate);
        handle_float_parameter(camera, "ExposureTime", config.exposure_time);
        handle_enum_parameter(camera, "AcquisitionMode", config.acquisition_mode);
...
int main(int argc, char* argv[])
{
        ...
        try
        {
                ...
                if (serial_number == "") {
                        ...
                        camera.Attach(tlFactory.CreateFirstDevice());
                } else {
